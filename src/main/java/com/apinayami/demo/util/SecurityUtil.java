...
public class SecurityUtil {
        ...
        private final long JWR_EXPIRATION_ACCESSTOKEN = 24 * 60 * 60; // 24 giờ (thay vì 10 phút)
        private final long JWR_EXPIRATION_REFRESHTOKEN = 7 * 24 * 60 * 60; // 7 ngày
        ...
        public String createToken(Authentication authentication) {
                ...
                JwtClaimsSet claims = JwtClaimsSet.builder().issuedAt(now).expiresAt(validity).claim("roles", role)
                                .claim("fullName", currentUserDetail.getFullName())
                                .claim("email", currentUserDetail.getUsername()).subject(authentication.getName())
                                .build();
        ...
        public String createAcessTokenForOauth(Role role, String fullName, String email) {
                ...
                JwtClaimsSet claims = JwtClaimsSet.builder().issuedAt(now).expiresAt(validity)
                                .claim("roles", List.of(role)).claim("fullName", fullName).claim("email", email)
                                .subject(email)
                                .build();
                JwsHeader jwsHeader = JwsHeader.with(JWT_ALGORITHM).build();
                return jwtEncoder.encode(JwtEncoderParameters.from(jwsHeader, claims)).getTokenValue();
